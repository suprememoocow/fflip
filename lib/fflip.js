...
var self = module.exports = {
	...
	_maxCookieAge: 900000,

	_useVetoVoting: false,
	...
	config: function(params) {
		...
		if ('maxCookieAge' in params) {
			self._maxCookieAge = params.maxCookieAge;
		}

		self._useVetoVoting = !!params.useVetoVoting;
	},
	...
	userHasFeature: function(user, featureName) {
		...
		if (self._useVetoVoting) {
			/**
			 * Vote yes (returns truthy value)
       * Veto no (returns false)
			 * Abstain (returns falsy value, like undefined/null)
			 */
			var hasVotes = false;
			for (var i = 0; i < criteriaArray.length; i++) {
				var cKey = criteriaArray[i];
				var c_data = featureCriteria[cKey];
				var c_func = self._criteria[cKey];
				var vote = c_func(user, c_data);
	...
	express_route: function(req, res, next) {
		...
		res.cookie('fflip', flags, { maxAge: self._maxCookieAge });
		res.json(200, {
			feature: name,
			action: action,
			status: 200,
